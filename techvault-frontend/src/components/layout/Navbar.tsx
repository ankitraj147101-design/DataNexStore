        ...
        <nav className="w-full px-3 sm:px-8 lg:px-12 py-2.5 sm:py-3.5 flex items-center justify-between gap-3 sm:gap-6">
            ...
            <div className="relative hidden md:block">
              ...
              {catMenuOpen && (
                <div
                  ...
                  className="absolute left-0 top-full mt-2 w-[480px] bg-white border border-slate-200 rounded-2xl p-5 shadow-2xl space-y-3 z-50 animate-in fade-in slide-in-from-top-2 duration-150"
                >
                  <div className="pb-2 border-b border-slate-100 flex items-center justify-between">
                    <span className="text-xs font-bold text-slate-800 font-mono flex items-center gap-1.5">
                      <Sparkles className="w-3.5 h-3.5 text-sky-600" />
                      <span>Hardware & Component Sections</span>
                    </span>
                    <Link
                      href="/products"
                  ...
                  <div className="space-y-2">
                    {INITIAL_CATEGORIES.map((cat) => (
                      <Link
                        ...
                        onClick={() => setCatMenuOpen(false)}
                        className="group flex items-start gap-3 p-2.5 rounded-xl hover:bg-slate-50 border border-transparent hover:border-slate-200 transition"
                      >
                        <div className="w-8 h-8 rounded-xl bg-slate-900 text-sky-400 flex items-center justify-center shrink-0 group-hover:bg-sky-600 group-hover:text-white transition">
                          <Layers className="w-4 h-4" />
                        </div>
                        <div className="flex-1">
                          <div className="text-xs font-bold text-slate-900 group-hover:text-sky-600 transition flex items-center justify-between">
                            <span>{cat.name}</span>
                            {cat.slug === 'hdd' && (
          ...
          <div className="hidden sm:flex flex-1 max-w-3xl relative">
            <form onSubmit={handleSearchSubmit} className="w-full relative">
              <input
                type="text"
                placeholder="Search WD 500GB, 1TB, 2TB, 3TB, 4TB Hard Drives, RAM, SSDs..."
                value={localQuery}
              ...
              <div className="grid grid-cols-1 gap-1.5">
                {INITIAL_CATEGORIES.map((cat) => (
                  <Link
                    ...
                    href={`/products?category=${cat.slug}`}
                    onClick={() => setMobileMenuOpen(false)}
                    className="p-2.5 rounded-xl text-xs font-semibold text-slate-800 bg-slate-50 hover:bg-sky-50 hover:text-sky-700 transition flex items-center justify-between"
                  >
                    <span>{cat.name}</span>
                    {cat.slug === 'hdd' && (
                      <span className="text-[10px] bg-emerald-100 text-emerald-800 font-mono font-bold px-2 py-0.5 rounded-full">
